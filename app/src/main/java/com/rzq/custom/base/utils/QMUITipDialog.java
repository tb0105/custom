...
public class QMUITipDialog extends Dialog {
    ...
    public void ModifyTipWord(String t)
    {
        if(tipView !=null)
            tipView.setText(t);
    }
    ...
    public static class Builder {
        ...
        public QMUITipDialog create(boolean cancelable) {
            ...
            if (mTipWord != null && mTipWord.length() > 0) {
                dialog.tipView = new TextView(mContext);
                ...
                dialog.tipView.setLayoutParams(tipViewLP);

                dialog.tipView.setEllipsize(TextUtils.TruncateAt.END);
                dialog.tipView.setGravity(Gravity.CENTER);
                dialog.tipView.setMaxLines(2);
                dialog.tipView.setTextColor(ContextCompat.getColor(mContext, R.color.white));
                dialog.tipView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
                dialog.tipView.setText(mTipWord);

                contentWrap.addView(dialog.tipView);
            }
            return dialog;
        }
